...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids, label_id, valid_ids=None, label_mask=None, label_start=None, label_end=None):
...
def readfile(filename):
    ...
    for line in f:
        if len(line)==0 or line.startswith('-DOCSTART') or line[0]=="\n":
            # 若当前line为句子末尾（长为0 或者 以-DOCSTART开头 或者 为\n）,
            # 将句子及标签加入data
            if len(sentence) > 0:
